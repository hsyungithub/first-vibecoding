...
```
.claude/
  agent-memory/    # 에이전트별 프로젝트 메모리 (memory: project 사용 시 자동 생성)
                   # agent-memory/{agent-name}/MEMORY.md 및 개별 메모리 파일
  agents/          # Claude Code 서브 에이전트 정의
    code-reviewer.md
    prd-to-roadmap.md
    sprint-close.md
    sprint-planner.md
  settings.json    # Claude Code 프로젝트 설정 (모델 등)
...
- 스프린트 검증 원칙 — **자동화 가능한 항목은 sprint-close 시점에 직접 실행**:
  - ✅ **자동 실행**: `npm run build` — 빌드 성공 여부 확인
  - ✅ **자동 실행**: Playwright MCP로 UI 기능 검증 (렌더링, 인터랙션, 애니메이션) — `npm run dev` 실행 중인 경우 sprint-close agent가 직접 실행
  - ✅ **자동 실행**: `browser_console_messages(level: "error")`로 콘솔 에러 없음 확인
  - ❌ **수동 필요**: 브라우저 UI 시각적 확인 (디자인, 색상, 레이아웃 세부사항 등)
  - ❌ **수동 필요**: Vercel/GitHub Pages 배포 — 배포 타이밍은 사용자가 결정
