...
- 각 Phase가 독립적으로 동작하고 시각적으로 검증 가능하도록 점진적으로 구현합니다.
...
**페이지 로드:**
- 초기 렌더링 상태 확인 (레이아웃, 주요 UI 요소 존재 여부)
- 콘솔 에러/경고 없음

**인터랙션:**
- 버튼/컨트롤 클릭 → 상태 변화 확인
- 입력 이벤트 → 즉각적인 UI 반응 확인
- 엣지 케이스 (빈 상태, 경계값 등) 처리 확인

**애니메이션/시각 효과:**
- 애니메이션 시작/종료 상태 스냅샷 비교
- `browser_wait_for`로 애니메이션 완료 후 최종 상태 확인

**반응형:**
- 데스크톱/모바일 레이아웃 전환 확인 (`browser_resize` 활용)
